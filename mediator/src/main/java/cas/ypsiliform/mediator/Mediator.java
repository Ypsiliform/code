    ...
    implements Runnable
{
    ...
    private Thenable<Map<Integer, Integer>> recursiveNegotiation(AgentProxy current,
                                                                 Map<Integer, Proposal> proposals)
    {
        ...
        current.sendSolutionProposals(request).then(response -> {
            ...
            Map<Integer, Proposal> nextIterationProposals =
                new HashMap<Integer, Proposal>();
            ...
            for ( int j = 0; j < response.getDemands().size(); j++ )
            {
                ...
                nextIterationProposals.put(j,
                                           new Proposal(proposals
                                               .get(j).solution,
                                                        demandVariant));
            }
            ...
            current.getChildIds().forEach(childId -> {
                nextIteration.add(recursiveNegotiation(agents.get(childId),
                                                       nextIterationProposals));
            });
        });

        Thenable<Map<Integer, Integer>> result =
            new Thenable<Map<Integer, Integer>>();
